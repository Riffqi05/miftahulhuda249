...
$id=$_GET['id'];
$blog = mysqli_fetch_assoc( mysqli_query($conn, "SELECT * FROM tb_blog WHERE id='$id'"));
...
<body class="bg-gray-100">
    <div class="min-h-screen flex">
        ...
        <div class="flex-1 p-6">    
            <h2 class="text-3xl font-semibold mb-6">Edit Postingan Blog</h2>
  ...
  <form action="update_blog.php?id=<?=$id?>" method="POST" enctype="multipart/form-data">
            <div class="mb-4">
                <label for="judul" class="block text-gray-700 mb-2">Judul Blog</label>
                <input type="text" name="judul" id="judul" value="<?= $judul;?>" class="w-full border border-gray-300 p-2 rounded"required>
            </div>
         
            <?php if($thumbnail!=""):?>
            <div id="preview-area" class="flex justify-center items-center">
            <div class="relative w-60 h-60">
                <img class="w-full h-full object-cover rounded-lg border-2 border-red-500" src="../assets/images/blog/<?=$thumbnail?>" alt="Image Preview">
        ...
        <?php endif?>
            ...
            <div class="mb-4">
                ...
                <textarea type="text" name="editor" id="editor" autofocus class="w-full border border-gray-300 p-2 rounded">
                <?= $content;?>"
                </textarea>
            </div>
            <div class="mb-4">
                ...
                <div class="flex">
                    ...
                    <input type="text" value="<?= $tags;?>" name="tags" id="blog_tag" class="hidden w-full border border-gray-300 p-2 rounded">
                    <button  id="add_tag" type="button"class="bg-blue-500 pl-5 pr-5 text-white">Tambah</button>
                </div>
            <div id="tags-area" class="w-100 flex mt-2 flex-wrap">
            <?php if($tags != ""): ?>
                <?php foreach($tg as $t): if($t!=""):?>
                    <button onclick='removeEl(this, "<?= $t ?>")' type="button" class="bg-blue-500 gap-5 mr-2 mt-2 hover:bg-blue-700 text-white hover:bg-red-500 hover:cursor-pointer font-bold py-2 px-4 rounded-full flex justify-center items-center">
                        <?= $t ?>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                        </svg>
            ...
            <div class="mb-4">
                ...
                <input type="text"value="<?= $url;?>" name="url" id="url" class="w-full border border-gray-300 p-2 rounded" required>
            </div>
